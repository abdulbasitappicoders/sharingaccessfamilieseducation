    ...
    <form action="{{route('admin.update_app_version_settings')}}" method="POST" >
        ...
        @foreach($versions as $version)
            ...
            <div class="mb-3">
                ...
                <input type="text" value="{{ $version->built_number }}" class="form-control" name="built{{ $version->id }}" id="exampleInputUsername1" autocomplete="off" placeholder="Built Number">

                <label for="exampleInputPassword1" class="form-label">App Version</label>
                <input type="text" value="{{ $version->app_version }}" class="form-control" name="version{{ $version->id }}" id="exampleInputUsername1" autocomplete="off" placeholder="App Version">
            </div>
        @endforeach
